...
import 'package:skeletonizer/skeletonizer.dart'; // استورد Skeletonizer
          ...
          child: BlocBuilder<HomeDataCubit, HomeDataState>(
            ...
            builder: (context, state) {
              ...
              return state.when(
                ...
                loading: () {
                  // Fake data عشان Skeletonizer يشتغل
                  final fakeDoctors = List.filled(
                    5,
                    DoctorFake(name: "اسم مؤقت", specialization: "تخصص مؤقت", address: "عنوان مؤقت"),
                  );
                ...
                success: (response, userName) {
                  final doctors = response.data?[id]?.doctors ?? [];
                  if (doctors.isEmpty) {
                    return Center(
...
// Fake class لازم تعمله مؤقتاً عشان skeletonizer يبني ال layout
class DoctorFake {
  final String name;
